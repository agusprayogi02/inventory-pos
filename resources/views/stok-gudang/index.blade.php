...
@section('content_header')
    <div class="row mb-2">
        <div class="col-sm-6">
            <h1>{{ __('adminlte::menu.stok_gudang') }}</h1>
        </div>
        <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
                <li class="breadcrumb-item"><a href="{{ route('home') }}">{{ __('adminlte::menu.home') }}</a></li>
                <li class="breadcrumb-item active">{{ __('adminlte::menu.stok_gudang') }}</li>
            </ol>
        </div>
    </div>
@endsection
...
@section('content')
    <x-adminlte.tool.modal id="add-stok-gudang" title="Tambah Stok Gudang" size="md" theme="success" icon="fas fa-plus"
        v-centered static-backdrop scrollable>
        <form action="{{ route('stok-gudang.store') }}" method="post" id="form-stok-gudang">
            @csrf
    ...
    <div class="row">
        <div class="col-12">
            <div class="card">
                ...
                <!-- /.card-header -->
                <div class="card-body">
                    <x-adminlte.tool.datatable id="stok-gudang-tables" :heads="['ID', 'Nama Bahan', 'Isi', 'Jumlah', 'Aksi']" :config="[
                        'columns' => [
                            ['data' => 'id'],
                            ['data' => 'nama'],
                            ['data' => 'isi'],
